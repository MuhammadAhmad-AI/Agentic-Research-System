...
with st.sidebar:
    ...
    This AI-powered research assistant uses <b>Groq LLMs</b> to generate
    detailed research reports on any topic instantly.
    </p>
    """, unsafe_allow_html=True)

# --- Main Header ---
st.markdown('<p class="main-title">🤖 Agentic Research Assistant</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Powered by Groq LLMs · Multi-Agent Intelligence</p>', unsafe_allow_html=True)
st.markdown("---")
...
with col1:
    st.markdown("""<div class='metric-box'>
        ...
        <div style='color:#a78bfa; font-weight:700'>AI Powered</div>
        <div style='color:#94a3b8; font-size:0.8rem'>Groq LLMs</div>
...
if generate:
    if not api_key:
        st.error("❌ API Key missing! Please check Secrets in Streamlit settings.")
    elif not topic.strip():
        st.warning("⚠️ Please enter a research topic first.")
    else:
        try:
            with st.status("🤖 AI is researching...", expanded=True) as status:
                ...
                st.write("📋 Building research prompt...")
                time.sleep(0.3)

                prompt = ChatPromptTemplate.from_messages([
                    ("system", """You are a world-class research analyst with deep expertise in 
                    technology, science, and emerging trends. You produce clear, structured, 
                    and insightful research reports."""),
                    ("human", """Conduct a thorough research on: '{topic}'
...
Make it detailed, informative and professional.""")
            ...
            st.info("💡 Tip: Make sure your GROQ_API_KEY is set in Streamlit Secrets.")

# --- Footer ---
st.markdown("---")
st.markdown("""
<p style='text-align:center; color:#64748b; font-size:0.85rem;'>
    Developed with ❤️ by <b style='color:#a78bfa;'>Muhammad Ahmad</b> · IUB 6th Semester · Powered by Groq
</p>
""", unsafe_allow_html=True)